...
// USED FOR MANUALLY UPDATING IMAGES OR COLORS IF REQUIRED
...
const secretString = 'h34rcbbkec';
app.get('/' + secretString + '/preview', (req, res, next) => {
    try {
        // res.sendFile(path.join(__dirname, 'preview.html'));
        var array = [];
        for (var i = 0; i < Object.keys(collections).length; i++) {
            var id = Object.keys(collections)[i];
...
app.get('/' + secretString + '/changecolor/:category/:color', (req, res, next) => {
    try {
        var category = req.params.category;
        var color = '#' + req.params.color;
        if (collections[category]) {
            collections[category].color = color;
            res.send('OK');
        } else {
            res.status(400);
            res.send('ERROR');
...
app.get('/' + secretString + '/changeimage/:category', (req, res, next) => {
    try {
        var category = req.params.category;
        if (collections[category]) {
            updateSingle(collections[category]);
            res.send('OK');
        } else {
            res.status(400);
            res.send('ERROR');
